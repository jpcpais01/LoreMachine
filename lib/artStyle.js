...
export function buildImagePrompt(description, tier, { usesReference = false } = {}) {
  ...
  if (usesReference) {
    lines.push(
      ``,
      `reference_image_note: an image of this character is attached ONLY so their species, body shape, outfit, and colors stay consistent. Do NOT copy the reference image's pose, camera angle, framing, lighting, or background — this must be a completely different scene, exactly as described in main_character above.`
    );
  }
