...
4. Write the requested number of **image_prompts** — each a vivid, purely visual/physical description (strict maximum 200 words) of the SAME character, for an AI image generator. The first image will be generated on its own; every other one will be generated using the first image as a visual reference, so: keep ONLY the character themselves identical across every prompt (their species/body shape, build, outfit, colors, and defining features must never change), and otherwise change EVERYTHING else as much as possible between variations — a completely different pose/action, a different background and environment, different lighting and composition. Do not repeat the same setting or stance twice. Do not include camera jargon, meta commentary, or references to "trading cards" — just describe what should be seen in the image. Do NOT use the highlight markup below inside image_prompts — keep those plain.
...
Invent a short character concept: a vivid physical/visual description plus a brief hint of their lore, personality, or what they do. Strict maximum 100 words total. Do not mention art style, cameras, or "trading cards" — just pitch the character.
